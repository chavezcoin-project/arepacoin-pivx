...
Clone the git repositories for arepacoin and gitian and then checkout the arepacoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/arepacoin-crypto/arepacoin.git
cd arepacoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the arepacoin repository
...
To build ArepaCoin (for Linux, OSX and Windows) just follow the steps under 'perform
...
```bash
    ...
    From https://github.com/arepacoin-crypto/arepacoin
...
For example:
```bash
URL=https://github.com/crowning-/arepacoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit arepacoin=${COMMIT} --url arepacoin=${URL} ../arepacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit arepacoin=${COMMIT} --url arepacoin=${URL} ../arepacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit arepacoin=${COMMIT} --url arepacoin=${URL} ../arepacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/arepacoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/arepacoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/arepacoin-build.assert
```
...
[arepacoin/gitian.sigs](https://github.com/arepacoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
