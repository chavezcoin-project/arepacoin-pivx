...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeArepacoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeArepacoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeArepacoinAmount = strAnonymizeArepacoinAmount.remove(strAnonymizeArepacoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeArepacoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeArepacoinAmount * COIN) nMaxToAnonymize = nAnonymizeArepacoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeArepacoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeArepacoinAmount));
        strAnonymizeArepacoinAmount = strAnonymizeArepacoinAmount.remove(strAnonymizeArepacoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeArepacoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeArepacoinAmount == 0) {
